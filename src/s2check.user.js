...
(function () {
	...
	function wrapper(pluginInfo) {
		...
		function loadOldSettings() {
			...
			if (!settings.colors.powerspotInner) {
				settings.colors.powerspotInner = defaultSettings.colors.powerspotInner;
				settings.colors.powerspotOuter = defaultSettings.colors.powerspotOuter;
			}
			...
			try {
				if (settings.thisIsPogo) {
					...
					if (originalRANGE_INDICATOR_COLOR != null) {
						window.RANGE_INDICATOR_COLOR = originalRANGE_INDICATOR_COLOR; // eslint-disable-line camelcase
					}
					// eslint-disable-next-line camelcase
		...
		function showS2Dialog() {
			...
			const html =
				...
				`<p><input type="checkbox" id="chkHighlightCandidates" /><label for="chkHighlightCandidates" title="1) Highlight level 17 cells with a pokéstop/gym.<br/>2) Show the amount of pokéstops needed for a new gym (in the center of level 14 cells).<br/>3) Highlight a border around level 14 cells that need 1-3 more pokéstops for a new gym.">Highlight Cells that might get a Gym</label></p>
                 ...
                 <p><input type="checkbox" id="chkSyncEnabled" /><label for="chkSyncEnabled" title="Automatically fetch the JSON below on load and merge in new Pokéstops/Gyms/Power Spots.">Auto-sync data from URL</label></p>
                 <p><label for="txtSyncUrl">Sync URL:</label> <input type="text" id="txtSyncUrl" style="width:90%" title="JSON URL to sync from, e.g. a raw.githubusercontent.com link"></p>
			...
			// Checkbox and URL for auto-sync
			const chkSyncEnabled = div.querySelector("#chkSyncEnabled");
			chkSyncEnabled.checked = settings.syncEnabled;
			chkSyncEnabled.addEventListener("change", (e) => {
				settings.syncEnabled = chkSyncEnabled.checked;
				saveSettings();
			});

			const txtSyncUrl = div.querySelector("#txtSyncUrl");
			txtSyncUrl.value = settings.syncUrl;
			txtSyncUrl.addEventListener("change", (e) => {
				settings.syncUrl = txtSyncUrl.value.trim();
				saveSettings();
			});
		...
		function editColors() {
			...
			const html =
				...
				selectRow
					...
					.replace("{{width}}", "") +
				selectRow
					.replace("{{title}}", `Cell ${poiCellLevel} with a gym or stop`)
			...
			if (zoom > 4) {
				...
				if (cellsCloseToThreshold) {
					...
					for (let missingStops = 3; missingStops >= 1; missingStops--) {
						...
						const opacity =
							settings.colors["missingStops" + missingStops].opacity;
						cellsCloseToThreshold[missingStops].forEach((cell) =>
							gridLayerGroup.addLayer(drawCell(cell, color, 3, opacity)),
						);
					}
				}
								...
								case 0:
									if (missingGyms <= 0) {
										cellLayerGroup.addLayer(
											fillCell(
												cell,
												settings.colors.cell14Filled.color,
												settings.colors.cell14Filled.opacity,
											),
										);
									}
									break;
			...
			const drawCellAndNeighbors = function (cell, color, width, opacity) {
				...
				if (!seenCells[cellStr]) {
					...
					if (isCellOnScreen(bounds, cell)) {
						...
						if (cellGroup) {
							countLayer.addLayer(
								writeInCell(cell, Object.keys(cellGroup.portals).length),
							);
						}
			...
			function dumpGroup(data, title, useHeader) {
				...
				data.sort(sortByName).forEach((portal) => {
					...
					if (title === "Pokestops" || title === "Portals") {
						...
						const votes =
							typeof portal.votes === "undefined" ? 0 : portal.votes;
						scoreData =
						...
						wrapper
							...
							.addEventListener("input", function () {
								const update =
									...
									(portal.photos === 0 || this.valueAsNumber === 0);
								portal.photos = this.valueAsNumber;
								updateScore(portal, wrapper);
								// any item getting photos edited should be a pokestop
								updateItemInObjectStoreStore(S2.db, POKESTOPS, portal);
								if (update) {
		...
		const KEY_LAST_SYNC = "plugin-pogo-lastsync";
			...
			items.forEach((item) => {
				...
				lastAdd.onsuccess = (e) => {
					...
					if (itemSuccessCallback && successCount % 2500 === 0) {
						itemSuccessCallback(objectStoreName + ": " + successCount);
					}
				};
			});
			if (lastAdd) {
		...
		function loadPogoS2Data(db) {
			...
			loadAllFromObjectStore(db, MANUAL_WAYPOINTS, (all) =>
				all.forEach((manualWaypoint) => {
					if (isNaN(manualWaypoint.latE6) || isNaN(manualWaypoint.lngE6)) {
						return;
					}
				...
				setTimeout(function () {
					...
					document
						...
						.addEventListener("change", (ev) => {
							...
							updateItemInObjectStoreStore(S2.db, GYMS, gyms[guid]);
							// update gym marker
							if (icon) {
        ...
        */
		...
		thisPlugin.pogoActionsDialog = function () {
			const content = `<div id="pogoSetbox">
                ...
                <a id="sync-now" title="Fetch the configured Sync URL now and merge in new data, ignoring the auto-sync interval">Sync now</a>
			...
			div
				...
				.addEventListener("click", (e) => saveDialog());
			div
				.querySelector("#sync-now")
				.addEventListener("click", (e) =>
					thisPlugin.syncFromUrl(settings.syncUrl, false),
				);
		};
		...
		function saveDialog() {
			...
			const container = dialog({
				...
				buttons: {
					Save: function () {
						...
						let filename =
							(SaveDataType === "Gyms" ? "gyms_" : "gyms+stops_") +
							new Date().toISOString().substr(0, 19).replace(/[\D]/g, "_");
						if (SaveDataFormat === "CSV") {
							...
							const data = {
								gyms: findPhotos(
									cleanUpExtraData(filterItemsByMapBounds(gyms)),
								),
							};
							if (SaveDataType !== "Gyms") {
								data.pokestops = findPhotos(
									cleanUpExtraData(filterItemsByMapBounds(pokestops)),
								);
							}
			...
			const div = container[0];
			div.querySelector("#PogoSaveDataType" + settings.saveDataType).checked =
				true;
			div.querySelector(
		...
		/**
		 * Parses a JSON string with pogo data (same shape produced by optExport) and merges
		 * new items into the local IndexedDB storage. Used both by manual file import and by
		 * the automatic sync from a remote URL.
		 * @param {string} content - JSON text to import
		 * @param {boolean} silent - if true, report progress/errors via console only (no dialog
		 * is guaranteed to be open, e.g. during automatic sync at startup)
		 */
		function processImportContent(content, silent) {
			const notify = silent
				? (message) => console.log("pogo sync: " + message) // eslint-disable-line no-console
				: (message) => thisPlugin.optAlert(message);
			...
			try {
				...
				Object.keys(list).forEach((type) => {
					...
					console.log(
						"adding " + itemsToAddForType.length + " items to " + type,
					);
					promises.push(
						addItemsToObjectStore(S2.db, type, itemsToAddForType, (message) =>
							notify("Importing " + message),
						),
					);
				});
				...
				notify('<span style="color: #f88">Import failed</span>');
			}
		}

		thisPlugin.optImport = function () {
			readFromFile(function (content) {
				processImportContent(content, false);
			});
		};

		/**
		 * Fetches a JSON file (same shape produced by optExport) from a remote URL and merges
		 * it into the local storage, same as a manual import. Used for the automatic sync from
		 * GitHub and for the manual "Sync now" action.
		 * @param {string} url - URL of the JSON file to fetch
		 * @param {boolean} silent - if true, don't rely on a dialog being open to show progress
		 */
		thisPlugin.syncFromUrl = function (url, silent) {
			...
			fetch(url)
				...
				.then((content) => {
					processImportContent(content, silent);
					localStorage[KEY_LAST_SYNC] = Date.now();
				})
				.catch((e) => {
					console.warn("pogo: failed to sync from " + url + ": " + e); // eslint-disable-line no-console
					if (!silent) {
						thisPlugin.optAlert('<span style="color: #f88">Sync failed</span>');
					}
				});
		};

		/**
		 * Automatically calls syncFromUrl with the configured settings.syncUrl, but only if
		 * auto-sync is enabled and the last sync happened more than SYNC_INTERVAL_MS ago.
		 */
		function autoSyncIfDue() {
			if (!settings.syncEnabled || !settings.syncUrl) {
				return;
			}
			const lastSync = parseInt(localStorage[KEY_LAST_SYNC], 10) || 0;
			if (Date.now() - lastSync < SYNC_INTERVAL_MS) {
				return;
			}
			thisPlugin.syncFromUrl(settings.syncUrl, true);
		}
		...
		thisPlugin.addStar = function (guid, lat, lng, name, type) {
			...
			if (type === POKESTOPS) {
				...
				const hasPhoto =
					typeof pokestop.photos === "undefined" || pokestop.photos > 0;
    ...
    `,
		...
		function removeNearbyCircle(guid) {
			...
			if (circles != null) {
				circles.forEach((circle) => nearbyLayerGroup.removeLayer(circle));
		...
		function promptToClassifyGyms(groups) {
			...
			const container = dialog({
				...
				buttons: {
					...
					"There is no Gym": function () {
						...
						setKeyInObjectStore(S2.db, MISSING_GYMS, cellData.cell.toString());
			...
			container.on("click", "a", function (e) {
				...
				if (missingGyms === 0) {
					...
					$(row).fadeOut(200);
					document.querySelector(".ui-dialog-title-active").textContent =
						missingGyms === 1
							? "Which one is a Gym?"
							: "Which " + missingGyms + " are Gyms?";
				}
		...
		function openS2DB() {
			...
			request.onupgradeneeded = function (event) {
				...
				if (event.oldVersion < 2) {
					...
					createAndLoadObjectStore(upgradeDb, GYMS, "guid", tmp, (os, data) => {
					...
					createAndLoadObjectStore(
						...
						tmp,
						(os, data) => {
							Object.keys(data.pokestops || {}).forEach((pokestop) => {
								os.add(data.pokestops[pokestop]);
								pokestops[pokestop] = data.pokestops[pokestop];
							});
						},
					);
					createAndLoadObjectStore(
						...
						tmp,
						(os, data) => {
							Object.keys(data.notpogo || {}).forEach((nothing) => {
					...
					createAndLoadObjectStore(
						...
						tmp,
						(os, data) => {
							Object.keys(data.ignoredCellsExtraGyms || {}).forEach(
								(ignoredCell) => {
					...
					createAndLoadObjectStore(
						...
						tmp,
						(os, data) => {
							Object.keys(data.ignoredCellsMissingGyms || {}).forEach(
								(ignoredCell) => {
									os.add(ignoredCell, ignoredCell);
									ignoredCellsMissingGyms[ignoredCell] = true;
								},
				...
				if (event.oldVersion < 3) {
					...
					createAndLoadObjectStore(
						...
						tmp,
						(os, data) => {
							Object.keys(data.powerspots || {}).forEach((powerspot) => {
								os.add(data.powerspots[powerspot]);
								powerspots[powerspot] = data.powerspots[powerspot];
							});
						},
					);
				}
			...
			titleInput.addEventListener("change", (ev) => {
				try {
					...
					if (asUrl.host === "campfire.onelink.me") {
						...
						rootNode.getElementById("pogo-s2-pokestopLatitude").value = lat;
						rootNode.getElementById("pogo-s2-pokestopLongitude").value = lng;
			...
			window.portalDetail.request = function (guid) {
				if (guid.includes("s2-pogo")) {
					...
					request.onsuccess = (event) => {
						...
						const portal = window.mapDataRequest.render.createPortalEntity(
							ent,
							"detailed",
						);
		...
		const setup = function () {
			...
			sidebarPogo.appendChild(
				createCounter("Missing wayspots", "missing", promptToRemovePokestops),
			);
			sidebarPogo.appendChild(
				createCounter("New Gyms", GYMS, promptToClassifyGyms),
			);
			sidebarPogo.appendChild(
				createCounter("Cells with extra Gyms", "extraGyms", promptToVerifyGyms),
			);
